...
bool modifyQPtoInit(struct ibv_qp *qp, RdmaContext *context) {
  ...
  Debug::notifyInfo("RDMA QP: qpn=%u type=%d -> INIT port=%u access=0x%x",
                    qp->qp_num, qp->qp_type, context->port,
                    attr.qp_access_flags);

  if (ibv_modify_qp(qp, &attr,
                    IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT |
                        IBV_QP_ACCESS_FLAGS)) {
    Debug::notifyError("RDMA QP: failed INIT qpn=%u errno=%d (%s)", qp->qp_num,
...
bool modifyQPtoRTR(struct ibv_qp *qp, uint32_t remoteQPN, uint16_t remoteLid,
  ...
  Debug::notifyInfo(
      "RDMA QP: qpn=%u type=%d -> RTR remote_qpn=%u mtu=%d gidIndex=%d",
      qp->qp_num, qp->qp_type, remoteQPN, attr.path_mtu, context->gidIndex);

  if (ibv_modify_qp(qp, &attr, flags)) {
    Debug::notifyError("RDMA QP: failed RTR qpn=%u errno=%d (%s)", qp->qp_num,
                       errno, strerror(errno));
    return false;
  }
  return true;
...
bool modifyQPtoRTS(struct ibv_qp *qp) {
  ...
  Debug::notifyInfo("RDMA QP: qpn=%u type=%d -> RTS max_rd_atomic=%u",
                    qp->qp_num, qp->qp_type, attr.max_rd_atomic);

  if (ibv_modify_qp(qp, &attr, flags)) {
    Debug::notifyError("RDMA QP: failed RTS qpn=%u errno=%d (%s)", qp->qp_num,
                       errno, strerror(errno));
    return false;
  }
  return true;
